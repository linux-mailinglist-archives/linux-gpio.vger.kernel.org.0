...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 344B33D7893
	for <lists+linux-gpio@lfdr.de>; Tue, 27 Jul 2021 16:34:15 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232123AbhG0OeO (ORCPT <rfc822;lists+linux-gpio@lfdr.de>);
        Tue, 27 Jul 2021 10:34:14 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:49388 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S236728AbhG0OeI (ORCPT
        <rfc822;linux-gpio@vger.kernel.org>); Tue, 27 Jul 2021 10:34:08 -0400
Received: from mail-wr1-x42b.google.com (mail-wr1-x42b.google.com [IPv6:2a00:1450:4864:20::42b])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 53420C061760
        for <linux-gpio@vger.kernel.org>; Tue, 27 Jul 2021 07:34:06 -0700 (PDT)
Received: by mail-wr1-x42b.google.com with SMTP id g15so15502351wrd.3
        for <linux-gpio@vger.kernel.org>; Tue, 27 Jul 2021 07:34:06 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=bgdev-pl.20150623.gappssmtp.com; s=20150623;
        h=from:to:cc:subject:date:message-id:in-reply-to:references
         :mime-version:content-transfer-encoding;
        bh=srF+1yANDG71txB7xw6FyJmMobn42pYuxrURdCQc1js=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=srF+1yANDG71txB7xw6FyJmMobn42pYuxrURdCQc1js=;
        b=
X-Gm-Message-State: AOAM5339i5G2uhZcaAgoW3jPNkS1uQaPwMAu0rmqvkIVlkDnnXWUxRCr
        duvbNTJZ0SLke3qYLkHVPRpmOg==
X-Google-Smtp-Source: ABdhPJx4CiP9BvL6zlJJTJIyJ7QX/ko5vHkuDKcrehI+OgS7MTTuAq+L3+M/2vmlBl58WGtBOfRLJQ==
X-Received: by 2002:a5d:44c7:: with SMTP id z7mr16200393wrr.286.1627396444037;
        Tue, 27 Jul 2021 07:34:04 -0700 (PDT)
Received: from debian-brgl.home ([2a01:cb1d:334:ac00:7d50:ff5:f5c1:e225])
        by smtp.gmail.com with ESMTPSA id j1sm3782729wrm.86.2021.07.27.07.34.03
...
Subject: [libgpiod][PATCH v3 3/3] bindings: cxx: implement C++ bindings for libgpiod v2.0
Date:   Tue, 27 Jul 2021 16:34:00 +0200
Message-Id: <20210727143400.32543-4-brgl@bgdev.pl>
...
---
 ...
 configure.ac                                |   1 +
 34 files changed, 2750 insertions(+), 1831 deletions(-)
...
+GPIOD_CXX_API const ::std::string& chip::get_name(void) const
 {
-	this->throw_if_noref();
+	this->_m_priv->throw_if_closed();
...
+GPIOD_CXX_API const ::std::string& chip::get_label(void) const
 {
-	this->throw_if_noref();
+	this->_m_priv->throw_if_closed();
...
+GPIOD_CXX_API unsigned int chip::get_num_lines(void) const
 {
-	this->throw_if_noref();
-
...
+}
+
+GPIOD_CXX_API edge_event::type edge_event::get_type(void) const
+{
...
+}
+
+GPIOD_CXX_API uint64_t edge_event::get_timestamp(void) const noexcept
+{
+	return ::gpiod_edge_event_get_timestamp(this->_m_priv->get_event_ptr());
+}
+
+GPIOD_CXX_API unsigned int edge_event::get_line_offset(void) const noexcept
+{
+	return ::gpiod_edge_event_get_line_offset(this->_m_priv->get_event_ptr());
+}
+
+GPIOD_CXX_API unsigned long edge_event::get_global_seqno(void) const noexcept
+{
+	return ::gpiod_edge_event_get_global_seqno(this->_m_priv->get_event_ptr());
+}
+
+GPIOD_CXX_API unsigned long edge_event::get_line_seqno(void) const noexcept
+{
...
+				::std::cout << chip.get_name() << " " << offset << ::std::endl;
 				return EXIT_SUCCESS;
 			}
 		}
...
-		return EXIT_FAILURE;
-	}
+	::std::cout << chip.get_name() << " - " << chip.get_num_lines() << " lines:" << ::std::endl;
 
-	for (const auto& entry: ::std::filesystem::directory_iterator("/dev/")) {
-		if (::gpiod::is_gpiochip_device(entry.path())) {
-			::gpiod::chip chip(entry.path());
+	for (unsigned int offset = 0; offset < chip.get_num_lines(); offset++) {
+		auto info = chip.get_line_info(offset);
+
+		::std::cout << "\tline ";
+		::std::cout.width(3);
+		::std::cout << info.get_offset() << ": ";
+
+		::std::cout.width(12);
+		::std::cout << (info.get_name().empty() ? "unnamed" : info.get_name());
+		::std::cout << " ";
 
-			::std::cout << chip.name() << " - " << chip.num_lines() << " lines:" << ::std::endl;
+		::std::cout.width(12);
+		::std::cout << (info.get_consumer().empty() ? "unused" : info.get_consumer());
+		::std::cout << " ";
...
-				::std::cout << lit.offset() << ": ";
+		::std::cout.width(8);
+		::std::cout << (info.get_direction() == ::gpiod::line_info::direction::INPUT ? "input"
+											     : "output");
+		::std::cout << " ";
...
-				::std::cout << " ";
+		::std::cout.width(10);
+		::std::cout << (info.is_active_low() ? "active-low" : "active-high");
...
+	if (event.get_type() == ::gpiod::edge_event::type::RISING_EDGE)
...
+	::std::cout << event.get_timestamp() / 1000000000;
 	::std::cout << ".";
-	::std::cout << event.timestamp.count() % 1000000000;
+	::std::cout << event.get_timestamp() % 1000000000;
 
-	::std::cout << " line: " << event.source.offset();
+	::std::cout << " line: " << event.get_line_offset();
...
+					  ::gpiod::line_config(::gpiod::line_config::direction::INPUT,
+							       ::gpiod::line_config::edge::BOTH));
...
+				::gpiod::line_config(::gpiod::line_config::direction::OUTPUT));
...
+	 * @return Number of lines.
+	 */
+	unsigned int get_num_lines(void) const;
+
+	/**
...
+	 */
+	uint64_t get_timestamp(void) const noexcept;
+
+	/**
...
+	 * @return Line offset.
+	 */
+	unsigned int get_line_offset(void) const noexcept;
+
+	/**
...
+	 */
+	unsigned long get_global_seqno(void) const noexcept;
+
+	/**
...
+	 */
+	unsigned long get_line_seqno(void) const noexcept;
+
+private:
+
+	edge_event(void);
...
+	 */
+	enum class event_clock : unsigned int
+	{
+		MONOTONIC = 1,
...
+	/**
+	 * @brief Constructor.
+	 * @param dir_val Global direction setting.
+	 * @param edge_val Global edge detection.
+	 * @param active_low Global active_low setting.
+	 * @param output_values Default output values.
+	 * @param bias_val Global bias setting.
+	 * @param drive_val Global drive setting.
+	 * @param debounce_period Global debounce period in microseconds.
+	 * @param clock_val Default event clock mode.
+	 */
+	line_config(direction dir_val = direction::INPUT, edge edge_val = edge::NONE,
+		    bool active_low = false, const line_value_mappings& output_values = no_output_values(),
+		    bias bias_val = bias::AS_IS, drive drive_val = drive::PUSH_PULL,
+		    unsigned long debounce_period = 0, event_clock clock_val = event_clock::MONOTONIC);
+
...
+	 * @param clock_val New event clock setting.
+	 * @param offsets Vector of line offsets to set the debounce period for.
+	 *                If empty, the setting will be global.
+	 */
+	void set_event_clock(event_clock clock_val,
+			     const line_offsets& offsets = line_offsets()) const;
+
+	/**
...
+	 */
+	unsigned int get_offset(void) const noexcept;
+
+	/**
...
+	 *         not used.
+	 */
+	const ::std::string& get_consumer(void) const noexcept;
+
+	/**
...
+	 * @return True if this line is "active-low", false otherwise.
+	 */
+	bool is_active_low(void) const noexcept;
+
+	/**
...
+	 */
+	bool is_debounced(void) const noexcept;
+
+	/**
...
+	 *         not debounced.
+	 */
+	unsigned long get_debounce_period(void) const noexcept;
+
+private:
+
+	line_info(void);
...
+namespace {
+
+const ::std::map<int, info_event::type> event_type_mapping = {
+	{ GPIOD_INFO_EVENT_LINE_REQUESTED,	info_event::type::LINE_REQUESTED },
+	{ GPIOD_INFO_EVENT_LINE_REQUESTED,	info_event::type::LINE_RELEASED },
+	{ GPIOD_INFO_EVENT_LINE_REQUESTED,	info_event::type::LINE_CONFIG_CHANGED }
+};
+
+} /* namespace */
+
...
+}
+
+GPIOD_CXX_API info_event::type info_event::get_type(void) const
+{
...
+}
+
+GPIOD_CXX_API uint64_t info_event::get_timestamp(void) const noexcept
+{
...
+namespace {
+
+const ::std::map<line_config::direction, int> direction_mapping = {
+	{ line_config::direction::AS_IS,	GPIOD_LINE_CONFIG_DIRECTION_AS_IS },
+	{ line_config::direction::INPUT,	GPIOD_LINE_CONFIG_DIRECTION_INPUT },
+	{ line_config::direction::OUTPUT,	GPIOD_LINE_CONFIG_DIRECTION_OUTPUT }
+};
+
+const ::std::map<line_config::edge, int> edge_mapping = {
+	{ line_config::edge::NONE,		GPIOD_LINE_CONFIG_EDGE_NONE },
+	{ line_config::edge::FALLING,		GPIOD_LINE_CONFIG_EDGE_FALLING },
+	{ line_config::edge::RISING,		GPIOD_LINE_CONFIG_EDGE_RISING },
+	{ line_config::edge::BOTH,		GPIOD_LINE_CONFIG_EDGE_BOTH }
+};
+
+const ::std::map<line_config::bias, int> bias_mapping = {
+	{ line_config::bias::AS_IS,		GPIOD_LINE_CONFIG_BIAS_AS_IS },
+	{ line_config::bias::DISABLED,		GPIOD_LINE_CONFIG_BIAS_DISABLED },
+	{ line_config::bias::PULL_UP,		GPIOD_LINE_CONFIG_BIAS_PULL_UP },
+	{ line_config::bias::PULL_DOWN,		GPIOD_LINE_CONFIG_BIAS_PULL_DOWN }
+};
+
+const ::std::map<line_config::drive, int> drive_mapping = {
+	{ line_config::drive::PUSH_PULL,	GPIOD_LINE_CONFIG_DRIVE_PUSH_PULL },
+	{ line_config::drive::OPEN_DRAIN,	GPIOD_LINE_CONFIG_DRIVE_OPEN_DRAIN },
+	{ line_config::drive::OPEN_SOURCE,	GPIOD_LINE_CONFIG_DRIVE_OPEN_SOURCE }
+};
+
+const ::std::map<line_config::event_clock, int> clock_mapping = {
+	{ line_config::event_clock::MONOTONIC,	GPIOD_LINE_CONFIG_EVENT_CLOCK_MONOTONIC },
+	{ line_config::event_clock::REALTIME,	GPIOD_LINE_CONFIG_EVENT_CLOCK_REALTIME },
+};
+
...
+}
+
+GPIOD_CXX_API line_config::line_config(direction dir_val, edge edge_val,
+		    bool active_low, const line_value_mappings& output_values,
+		    bias bias_val, drive drive_val, unsigned long debounce_period,
+		    event_clock clock_val)
+	: _m_priv(new impl)
+{
+	this->set_direction(dir_val);
+	this->set_edge_detection(edge_val);
+	this->set_bias(bias_val);
+	this->set_drive(drive_val);
+	active_low ? this->set_active_low() : this->set_active_high();
+	this->set_debounce_period(debounce_period);
+	this->set_event_clock(clock_val);
...
+}
+
+GPIOD_CXX_API void line_config::set_direction(direction dir_val, const line_offsets& offsets) const
+{
+	set_mapped_value<direction, ::gpiod_line_config_set_direction,
+			 ::gpiod_line_config_set_direction_subset>(this->_m_priv->config.get(),
+								   dir_val, offsets,
+								   direction_mapping);
+}
+
+GPIOD_CXX_API void line_config::set_edge_detection(edge edge_val, const line_offsets& offsets) const
+{
+	set_mapped_value<edge, ::gpiod_line_config_set_edge_detection,
+			 ::gpiod_line_config_set_edge_detection_subset>(
+				this->_m_priv->config.get(), edge_val, offsets, edge_mapping);
+}
+
+GPIOD_CXX_API void line_config::set_bias(bias bias_val, const line_offsets& offsets) const
+{
+	set_mapped_value<bias, ::gpiod_line_config_set_bias,
+			 ::gpiod_line_config_set_bias_subset>(this->_m_priv->config.get(),
+							      bias_val, offsets, bias_mapping);
+}
+
+GPIOD_CXX_API void line_config::set_drive(drive drive_val, const line_offsets& offsets) const
+{
+	set_mapped_value<drive, ::gpiod_line_config_set_drive,
+			 ::gpiod_line_config_set_drive_subset>(this->_m_priv->config.get(),
+							       drive_val, offsets, drive_mapping);
+}
+
...
+}
+
+GPIOD_CXX_API void line_config::set_event_clock(event_clock clock_val,
+						const line_offsets& offsets) const
+{
+	set_mapped_value<event_clock, ::gpiod_line_config_set_event_clock,
...
+namespace {
+
+const ::std::map<int, line_info::direction> direction_mapping = {
+	{ GPIOD_LINE_DIRECTION_INPUT,	line_info::direction::INPUT },
+	{ GPIOD_LINE_DIRECTION_OUTPUT,	line_info::direction::OUTPUT }
+};
+
+const ::std::map<int, line_info::bias> bias_mapping = {
+	{ GPIOD_LINE_BIAS_UNKNOWN,	line_info::bias::UNKNOWN },
+	{ GPIOD_LINE_BIAS_DISABLED,	line_info::bias::DISABLED },
+	{ GPIOD_LINE_BIAS_PULL_UP,	line_info::bias::PULL_UP },
+	{ GPIOD_LINE_BIAS_PULL_DOWN,	line_info::bias::PULL_DOWN }
+};
+
+const ::std::map<int, line_info::drive> drive_mapping = {
+	{ GPIOD_LINE_DRIVE_PUSH_PULL,	line_info::drive::PUSH_PULL },
+	{ GPIOD_LINE_DRIVE_OPEN_DRAIN,	line_info::drive::OPEN_DRAIN },
+	{ GPIOD_LINE_DRIVE_OPEN_SOURCE,	line_info::drive::OPEN_SOURCE }
+};
+
+const ::std::map<int, line_info::edge> edge_mapping = {
+	{ GPIOD_LINE_EDGE_NONE,		line_info::edge::NONE },
+	{ GPIOD_LINE_EDGE_RISING,	line_info::edge::RISING },
+	{ GPIOD_LINE_EDGE_FALLING,	line_info::edge::FALLING },
+	{ GPIOD_LINE_EDGE_BOTH,		line_info::edge::BOTH }
+};
+
+} /* namespace */
+
...
+}
+
+GPIOD_CXX_API unsigned int line_info::get_offset(void) const noexcept
+{
+	return ::gpiod_line_info_get_offset(this->_m_priv->info.get());
+}
+
+GPIOD_CXX_API const ::std::string& line_info::get_name(void) const noexcept
+{
+	return this->_m_priv->name;
+}
+
+GPIOD_CXX_API bool line_info::is_used(void) const noexcept
+{
+	return ::gpiod_line_info_is_used(this->_m_priv->info.get());
+}
+
+GPIOD_CXX_API const ::std::string& line_info::get_consumer(void) const noexcept
+{
+	return this->_m_priv->consumer;
+}
+
+GPIOD_CXX_API line_info::direction line_info::get_direction(void) const
+{
...
+}
+
+GPIOD_CXX_API bool line_info::is_active_low(void) const noexcept
+{
+	return ::gpiod_line_info_is_active_low(this->_m_priv->info.get());
+}
+
+GPIOD_CXX_API line_info::bias line_info::get_bias(void) const
+{
...
+}
+
+GPIOD_CXX_API line_info::drive line_info::get_drive(void) const
+{
...
+}
+
+GPIOD_CXX_API line_info::edge line_info::get_edge_detection(void) const
+{
...
+}
+
+GPIOD_CXX_API bool line_info::is_debounced(void) const  noexcept
+{
+	return ::gpiod_line_info_is_debounced(this->_m_priv->info.get());
+}
+
+GPIOD_CXX_API unsigned long line_info::get_debounce_period(void) const  noexcept
+{
